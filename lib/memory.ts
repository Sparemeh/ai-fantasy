...
export class MemoryManager {
    ...
    public async vectorSearch(
        ...
        const similarDocs = await vectorStore.similaritySearch(recentChatHistory, 3, { fileName: characteFileName }).catch((err) => {
    ...
    public static async getInstance(): Promise<MemoryManager> {
        if (!MemoryManager.instance) {
            if (!process.env.PINECONE_API_KEY) {
                throw new Error("PINECONE_API_KEY is not defined in the environment.");
            }
